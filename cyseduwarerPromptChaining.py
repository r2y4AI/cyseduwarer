...
def process_question(question: str, vector_db: Chroma, selected_model: str) -> str:
    ...
    control_names_template = """
    You are a question-answering system for a self-learning of cybersecurity education and awareness. 
    Your first task is to extract the controls that are listed in the ISO/IEC 27002:2022 standard and relevant to the learner question based on the following context: 
    {context}
    Precisely, output only the list of controls.
    Question: {question}
    """

    control_names_prompt = PromptTemplate(
    input_variables=["question", "context"],
    template=control_names_template
    )

    controls_chain = LLMChain(
    llm=llm,
    prompt=control_names_prompt,
    output_key="controlnames",
    output_parser=StrOutputParser()
    )

    control_details_template = """
    You are a question-answering system for a self-learning of cybersecurity education and awareness. 
    Your second task is to answer a given learner question about the control that is listed in ISO/IEC 27002:2022 standard. 
    ...
    Given the list of controls from the first task, your task is to list these five attributes alongside the corresponding attributes values for each control precisely. 
    ...
    - Purpose
    - Guidance 
    The list of control names: {controlnames}
    """
    
    control_details_prompt = PromptTemplate(
    input_variables=["controlnames", "context"],
    template=control_details_template
    )

    details_chain = LLMChain(
    llm=llm,
    prompt=control_details_prompt,
    output_key="details",
    output_parser=StrOutputParser()
    )

    overall_chain = SequentialChain(
    chains=[controls_chain, details_chain],
    input_variables=["question", "context"],
    output_variables=["controlnames", "details"],
    verbose=True
    )
    ...
    response = overall_chain.invoke({"question": question, "context": context})
